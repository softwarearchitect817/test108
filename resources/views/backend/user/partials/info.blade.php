    ...
    <div class="box box-primary">
        <div class="box-body box-profile">
            <img class="profile-user-img img-responsive img-circle"
                src="/public/back/img/{{ $user->present()->role_id }}.png" alt="{{ $user->present()->username }}">
            ...
            <ul class="list-group list-group-unbordered">
                ...
                @if ($user->hasRole('user'))
                    <li class="list-group-item">
                        <b>@lang('app.in')</b> <a
                            class="pull-right">{{ number_format($user->present()->total_in, 2) }}</a>
                    </li>
                    <li class="list-group-item">
                        <b>@lang('app.out')</b> <a
                            class="pull-right">{{ number_format($user->present()->total_out, 2) }}</a>
                    </li>
                    <li class="list-group-item">
                        <b>@lang('app.total')</b> <a
                            class="pull-right">{{ number_format($user->present()->total_in - $user->present()->total_out, 2) }}</a>
                    </li>
                @endif
            ...
            @if ($user->id != Auth::id())
                @if (
                    ...
                    @permission('users.delete')
                        <a href="{{ route('backend.user.delete', $user->id) }}" class="btn btn-danger btn-block"
                            data-method="DELETE" data-confirm-title="@lang('app.please_confirm')"
                            data-confirm-text="@lang('app.are_you_sure_delete_user')" data-confirm-delete="@lang('app.yes_delete_him')">
                ...
                @permission('users.delete')
                    @if (auth()->user()->hasRole('admin') && $user->hasRole(['agent', 'distributor']))
                        <a href="{{ route('backend.user.hard_delete', $user->id) }}" class="btn btn-danger btn-block"
                            data-method="DELETE" data-confirm-title="@lang('app.please_confirm')"
                            data-confirm-text="@lang('app.are_you_sure_delete_user')" data-confirm-delete="@lang('app.yes_delete_him')">
        ...
        <div>
            ...
            @if (
                ...
                <button type="button" class="btn btn-block btn-success btn-xs newPayment addPayment"
                    ...
                    data-id="{{ $user->id }}">@lang('app.add')</button>
            @else
            ...
            @if (
                ((Auth::user()->hasRole('admin') && $user->hasRole(['agent'])) ||
                    ...
                    (Auth::user()->hasRole('cashier') && $user->hasRole('user'))) &&
                    !(
                        $user->count_tournaments > 0 ||
                        $user->count_happyhours > 0 ||
                        $user->count_refunds > 0 ||
                        $user->count_progress > 0 ||
                        $user->count_daily_entries > 0 ||
                        $user->count_invite > 0 ||
                        $user->count_welcomebonus > 0 ||
                        $user->count_smsbonus > 0 ||
                        $user->count_wheelfortune > 0 ||
                ...
                <button type="button" class="btn btn-block btn-danger btn-xs newPayment outPayment" data-toggle="modal"
                    data-target="#openOutModal" data-id="{{ $user->id }}">@lang('app.out')</button>
            @else
        ...
        <div class="box box-primary">
            ...
            <div class="box-body">
                ...
                <div class="form-group">
                    ...
                    {!! Form::select('google2fa_enable', [0 => __('app.disabled'), 1 => __('app.active')], $user->google2fa_enable, [
                        'class' => 'form-control',
                    ]) !!}
