                                                        ...
                                                        <tr>
                                                            <td>
                                                                @if ($stat->payeer && $stat->payeer->hasRole(['cashier']))
                                                                    <a
                                                                        href="{{ route('backend.user.edit', $stat->payeer->id) }}">
                                                ...
                                                @if (count($gamestat))
                                                    @foreach ($gamestat as $stat)
                                                        <tr>
                                                            <td>
                                                                <a
                                                                    href="{{ route('backend.game_stat', ['game' => $stat->game]) }}">
                                                                    {{ $stat->game }}
                                                                </a>
                                                            </td>
                                                            <td>
                                                                <a
                                                                    href="{{ route('backend.game_stat', ['user' => $stat->user ? $stat->user->username : '']) }}">
                                                            ...
                                                            <td>{{ $stat->bet }}</td>
                                                            <td>{{ $stat->win }}</td>
                                                            <td>{{ date(config('app.time_format'), strtotime($stat->date_time)) }}
                                                            </td>
                                                        </tr>
                                                    @endforeach
                                                @else
                                                ...
                                                @if (count($open_shift))
                                                    @foreach ($open_shift as $num => $stat)
                                                        <tr>
                                                            ...
                                                            <td>{{ date(config('app.date_time_format'), strtotime($stat->start_date)) }}
                                                            </td>
                                                            <td>{{ $stat->end_date ? date(config('app.date_time_format'), strtotime($stat->end_date)) : '' }}
                                                            </td>
                                                            @if (!auth()->user()->hasRole('cashier'))
                                                                <td>{{ $stat->balance }}</td>
                                                            ...
                                                            @endif
                                                            <td>{{ number_format($stat->balance + $stat->balance_in - $stat->balance_out, 4, '.', '') }}
                                                            </td>
                                                            @permission('games.in_out')
                                                                @php
                                                            ...
                                                            @endpermission
                                                            <td>{{ number_format(!$stat->end_date ? $stat->get_jpg() : $stat->jpg, 4, '.', '') }}
                                                            </td>
                                                            @if (!auth()->user()->hasRole('cashier'))
                                                                <td>
                    ...
                    <div class="content-i content-i-2">
                        <div class="element-wrapper element-wrapper-2">
                            <div class="rowrightdiv text-center">
                                ...
                                <div class="col-sm-12 b-r b-b">
                                    <div class="el-tablo centered el-tabloPiso">
                                        <div class="value text-success statsTop top_in">{{ number_format($stats['new']) }}
                                        </div>
                                ...
                                <div class="col-sm-12 b-r b-b">
                                    <div class="el-tablo centered el-tabloPiso">
                                        <div class="value text-danger statsTop top_out">
                                            {{ number_format($stats['banned']) }}</div>
                                ...
                                <div class="col-sm-12 b-r b-b">
                                    <div class="el-tablo centered el-tabloPiso">
                                        ...
                                        <div class="value text-primary statsTop top_total">
                                            {{ number_format($stats['games']) }}</div>
                            ...
                            <div class="rowrightdiv text-center mt-4">
                                <div class="col-md-12">
                                    <a class="btn btn-success" href="{{ route('backend.start_shift') }}">
                                    ...
                                    @if (Auth::user()->shop->is_blocked)
                                        @permission('shops.unblock')
                                            <div class="col-md-12"><a class="btn btn-danger"
                                                    href="{{ route('backend.settings.shop_unblock') }}" data-method="PUT"
                                                    data-confirm-title="@lang('app.please_confirm')"
                                                    data-confirm-text="@lang('app.are_you_sure_unblock_shop')"
                                                    data-confirm-delete="@lang('app.unblock')"> UnBlock Shop</a></div>
                                        @endpermission
                                    @else
                                        @permission('shops.block')
                                            <div class="col-md-12"><a class="btn btn-danger"
                                                    href="{{ route('backend.settings.shop_block') }}" data-method="PUT"
                                                    data-confirm-title="@lang('app.please_confirm')"
                                                    data-confirm-text="@lang('app.are_you_sure_block_shop')"
                                                    data-confirm-delete="@lang('app.block')"> Block Shop</a></div>
                                        @endpermission
                                    @endif
                                @endif
                                @if (Auth::user()->hasRole('distributor') &&
                                        auth()->user()->present()->shop)
                                    <div class="col-md-12">
                                        <a class="btn btn-link"
                                            href="{{ route('backend.shop.action', [auth()->user()->present()->shop,'jpg_out']) }}"
                                    ...
                                    <div class="col-md-12">
                                        <a class="btn btn-link"
                                            href="{{ route('backend.shop.action', [auth()->user()->present()->shop,'games_out']) }}"
                                            data-method="DELETE" data-confirm-title="@lang('app.please_confirm')"
                                    ...
                                    <div class="col-md-12">
                                        <a class="btn btn-link"
                                            href="{{ route('backend.shop.action', [auth()->user()->present()->shop,'return_out']) }}"
                                            data-method="DELETE" data-confirm-title="@lang('app.please_confirm')"
...
@section('scripts')
    {!! HTML::script('/public/back/dist/js/pages/dashboard.js') !!}
@stop
