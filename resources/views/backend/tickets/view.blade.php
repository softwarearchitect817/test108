                ...
                <div class="box box-warning direct-chat direct-chat-warning">
                    <div class="box-header with-border">
                        ...
                        @if (auth()->user()->hasRole('admin') && !$ticket->admin)
                            ...
                            Shop={{ $ticket->shop ? $ticket->shop->name : 'No Shop' }} / IP={{ $ticket->ip_address }} /
                            Country={{ $ticket->country }} / City={{ $ticket->city }} / OS={{ $ticket->os }} /
                            Device={{ $ticket->device }} / Browser={{ $ticket->browser }}
                        @endif
                        <p>{!! $ticket->text !!}</p>
                    </div>
                    <div class="box-body">
                                ...
                                @if (!$answer->user->hasRole('admin'))
                                    <div class="direct-chat-msg">
                                        <div class="direct-chat-info clearfix">
                                            <span class="direct-chat-name pull-left">{{ $answer->user->username }}
                                                @if (auth()->user()->hasRole('admin'))
                                                    /Role={{ $answer->user->role->name }} / Shop={{ $ticket->shop->name }}
                                                    / IP={{ $answer->ip_address }} / Country={{ $answer->country }} /
                                                    City={{ $answer->city }} / OS={{ $answer->os }} /
                                                    Device={{ $answer->device }} / Browser={{ $answer->browser }}
                                                @endif
                                            </span>
                                        ...
                                        <img class="direct-chat-img"
                                            src="/public/back/img/{{ $answer->user->role_id }}.png">
